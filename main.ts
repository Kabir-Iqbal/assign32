...
// accepted.
let current_users = ["Ali","Übaid","Kaser","Rahib","Razaq"]
let new_users = ["Hashim","Hussain","Rehan","Ali","Kaser"]
// we will use to for each method 
new_users.forEach(new_user =>{
    let newuserlower = new_user.toLowerCase();

    // we wil use to some method to connect lists
    let alreadytaken = current_users.some(current_user => current_user.toLowerCase() === newuserlower)
    if (alreadytaken) {
        console.log(`${new_user} is already taken try another username.`);
        
    }else{
        console.log(`${new_user} is added to username.`);
